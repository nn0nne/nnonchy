...
# ---- Arch update checker (pacman + paru) ----
UPDATE_CACHE="$HOME/.cache/arch_updates"
REBOOT_FLAG="$HOME/.cache/reboot_required"
LOCK_FILE="$HOME/.cache/arch_updates.lock"
...
check_reboot_required() {
    ...
    for kernel in linux linux-lts linux-zen linux-hardened; do
        installed_kernel=$(pacman -Q "$kernel" 2>/dev/null | awk '{print $2}')
        if [[ -n "$installed_kernel" ]]; then
            installed_base=${installed_kernel%%-*}
            if [[ "$installed_base" != "$current_base" ]]; then
                echo 1 > "$REBOOT_FLAG"
                return
            fi
        fi
...
check_arch_updates() {
    ...
    if [[ -f "$UPDATE_CACHE" ]]; then
        ...
        current_time=$(date +%s)
        if [[ $((current_time - last_update)) -le $UPDATE_INTERVAL ]]; then
            check_reboot_required
            return
        fi
    fi
    
    # Simple lock to prevent multiple concurrent checks
    if [[ -f "$LOCK_FILE" ]]; then
        lock_time=$(stat -c %Y "$LOCK_FILE" 2>/dev/null || echo 0)
        if [[ $(($(date +%s) - lock_time)) -gt 600 ]]; then
            rm -f "$LOCK_FILE"
        else
            return
        fi
    ...
    # Count updates (handle empty output)
    repo=$(echo -n "$repo_list" | grep -c '^' 2>/dev/null || echo 0)
    aur=$(echo -n "$aur_list" | grep -c '^' 2>/dev/null || echo 0)
    
    # Check for important updates
    important=0
    if command -v rg >/dev/null 2>&1; then
        echo "$repo_list" | rg -q '^(linux|glibc|systemd|linux-lts|linux-zen)\b' && important=1
    else
        echo "$repo_list" | grep -qE '^(linux|glibc|systemd|linux-lts|linux-zen)\b' && important=1
    fi
    
    echo "$repo $aur $important" > "$UPDATE_CACHE"
    rm -f "$LOCK_FILE"
...
show_arch_updates() {
    if [[ -f "$UPDATE_CACHE" ]]; then
        read repo aur important < "$UPDATE_CACHE" 2>/dev/null || return
        ...
        [[ "$repo" =~ ^[0-9]+$ ]] || repo=0
        [[ "$aur" =~ ^[0-9]+$ ]] || aur=0
        [[ "$important" =~ ^[0-9]+$ ]] || important=0
        
        icon="󰮯"
        
        if [[ $repo -gt 0 || $aur -gt 0 ]]; then
            msg="$icon Updates: "
            [[ $repo -gt 0 ]] && msg+="$repo official"
            [[ $repo -gt 0 && $aur -gt 0 ]] && msg+=" + "
            [[ $aur -gt 0 ]] && msg+="$aur AUR"
        else
            msg="$icon System up to date"
        fi
        
        [[ $important -eq 1 ]] && msg+=" ⚠ Critical"
        [[ -f "$REBOOT_FLAG" ]] && msg+=" 🔁 Reboot needed"
        
        echo "$msg"
    fi
}

# Run in background without leaving zombie processes
(check_arch_updates &) 2>/dev/null
show_arch_updates
